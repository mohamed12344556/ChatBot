...
class _ChatPage2State extends State<ChatPage2> {
  ...
  Future<void> uploadFile(File file) async {
    var request = http.MultipartRequest(
      'POST',
      Uri.parse('https://api-chat-with-docs.onrender.com/upload/'),
    );
  ...
  Future<void> questionFromUserWithUrl(String question, String url) async {
    final uri = Uri.parse("https://api-chat-with-docs.onrender.com/chaturl/?url=$url");
                    ...
                    onPressed: () {
                      setState(() {
                        if (_chatController.text.isNotEmpty) {
                          ...
                          String url = "https://example.com"; // رابط URL المراد استفسار عنه
                          _chatHistory.add({
                            "message": question,
                            "isSender": true,
                          });
                          ...
                          questionFromUserWithUrl(
                              question, url); // Send the question to the server
                        }
                      });
                        ...
                        child: IconButton(
                          onPressed: () {
                            setState(() {
                              if (_chatController.text.isNotEmpty) {
                                String question = _chatController.text;
                                String url = "https://ar.wikipedia.org/wiki/%D9%85%D8%B1%D9%83%D8%A8_%D8%B9%D8%B6%D9%88%D9%8A"; // رابط URL المراد استفسار عنه
                                _chatHistory.add({
                                  "message": question,
                                  "isSender": true,
                                });
